...
bool eliminate_row(unsigned int sudoku[9][9], int row_index)
{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        for (int j = 0; j < 9; j++) {
            ...
            if (!(is_number(sudoku[row_index][j]))) {
                continue;
            }
...
bool eliminate_col(unsigned int sudoku[9][9], int col_index)
{
    bool changed = false;
    for (int i = 0; i < 9; i++) {
        if (is_number(sudoku[i][col_index])) {
            continue;
        }
        ...
        for (int j = 0; j < 9; j++) {
            ...
            if (!(is_number(sudoku[j][col_index]))) {
                continue;
            }
...
bool needs_solving(unsigned int sudoku[9][9])
{
    for (int row = 0; row < 9; row++) {
        for (int collum = 0; collum < 9; collum++) {
            if (!(is_number(sudoku[row][collum]))) {
                return 1;
            }
        }
    }
...
bool is_valid(unsigned int sudoku[9][9])
{
    for (int row = 0; row < 9; row++) {
        for (int collum = 0; collum < 9; collum++) {
            if (sudoku[row][collum] == 0 && possible_number(row, collum, sudoku) == 0) {
                return 0;
            }
            if (is_number(sudoku[row][collum]) && !(check_number(row, collum, sudoku))) {
                return 0;
            }
        }
    }
...
bool check_number(int row, int collum, unsigned int sudoku[9][9])
{
    ...
    while (row_block <= repeat_row) {
        while (col_block <= repeat_col) {
            if (row_block != row && col_block != collum && number == sudoku[row_block][col_block]) {
                return false;
            }
            col_block++;
        }
...
unsigned int possible_number(int row, int collum, unsigned int sudoku[9][9])
{
    ...
    for (int i = 0; i < 9; i++) {
        if (i != collum) {
            possible_row = possible_row ^ sudoku[row][i];
        }
        if (i != row) {
            possible_collum = possible_collum ^ sudoku[i][collum];
        }
    }
...
bool is_unpredictable(unsigned int number)
{
    if (!(is_number(number)) && !(is_impossible(number))) {
        return true;
    }
    return false;
}
